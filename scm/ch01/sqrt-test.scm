...
(test "sqrt2"
      (lambda ()
        ...
        (assert-eq (sqrt2 1)
                   1.0)
        (assert-eq (sqrt2 2)
                   1.414213562373095) ; good-enough1? and good-enough2?: 1.4142135623746899
        (assert-eq (sqrt2 9)
                   3.0)
        (assert-eq (sqrt2 (+ 100 37))
                   11.704699910719626)
        (assert-eq (sqrt2 (+ (sqrt2 2) (sqrt2 3)))
                   1.773771228186423) ; good-enough1? and good-enough2?: 1.7737712281868727
        (assert-eq (square (sqrt2 1000))
                   1000.0) ; good-enough1? and good-enough2?: 1000.0000000000343
        ;; Test for very small and very large numbers.
        (assert-eq (sqrt2 0.000001)
                   0.001) ; good-enough1? and good-enough2?: 0.0010000000000000117
