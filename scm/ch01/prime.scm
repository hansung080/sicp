...
(define (primes1-between/by lower upper prime?)
  ...
  (collect (if (and (<= lower 2) (>= upper 2) (prime? 2))
               '(2)
               '())
           (if (odd?_ lower)
               lower
...
;;   ...
;;
(define (composites-up-to limit)
  (let ((composites (make-vector (+ limit 1) #f))
        (half (quotient limit 2)))
    (define (sieve i)
...
(define (primes2-between/list lower upper)
  (let ((lower (if (< lower 2) 2 lower))
    ...
    (define (prime? n)
      (not (vector-ref composites n)))
    (define (collect primes i)
      (cond ((> i upper) (reverse primes))
            ((prime? i) (collect (cons i primes) (+ i 1)))
            (else (collect primes (+ i 1)))))
    (collect '() lower)))
...
(define (primes2-between/vector lower upper)
  (let ((lower (if (< lower 2) 2 lower))
    ...
    (define (count-primes count i)
      (cond ((> i upper) count)
            ((prime? i) (count-primes (+ count 1) (+ i 1)))
            (else (count-primes count (+ i 1)))))
    (let ((primes (make-vector (count-primes 0 lower))))
      (define (store-primes i j)
        (cond ((> i upper) primes)
              ((prime? i)
               (vector-set! primes j i)
               (store-primes (+ i 1) (+ j 1)))
              (else (store-primes (+ i 1) j))))
